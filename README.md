...
$$
\begin{align}
    \underset{\mathbf{u}}{\min}V_{N}(x,x_{r};\mathbf{u}) = \mathbf{u}^T(G_u \cdot Q \cdot G_u + R )\mathbf{u} + 2( \cdot x_{ k}^{T} \cdot G_{x}^{T} \cdot Q \cdot G_u)\mathbf{u}-2(\cdot x_{ R}^{T} \cdot Q \cdot G_u - u_{R}^{T} \cdot R )\mathbf{u}\\
\end{align}
$$
